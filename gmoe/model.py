...
def init_lora_layer_weight(
        layer: LLMDecoder, args: LLMModelArgs, config: Union[LoraConfig, MixConfig], index, weight: Optional[Dict[str, torch.Tensor]]):
    ...
    if isinstance(config, MixConfig):
        if config.sparse_step_ is None or layer.layer_id_ % config.sparse_step_ == 1:
            ...
            if config.routing_strategy_ == "goe":
                gate_layer_name = f"mixlora.layers.{layer.layer_id_}.gate."
                layer.mlp_.init_moe_weight(args=args, config=config, index = index,
                                        gate=None if weight is None else (weight, gate_layer_name))
            else:
                gate_layer_name = f"mixlora.layers.{layer.layer_id_}.gate.weight"
                layer.mlp_.init_moe_weight(args=args, config=config, index = index,
                                        gate=None if weight is None else weight[gate_layer_name])
            moe_layer_name_list = list(layer.mlp_.state_dict().keys())  # w1_proj, w2_proj, w3_proj
        else:
            moe_layer_name_list = []
    ...
    for idx, layer_name in enumerate(linear_layer_name_list):
        if layer_name in target and target[layer_name]:
            if init_moe and layer_name in moe_layer_name_list:
                for expert_idx in range(config.num_experts_ if isinstance(config.num_experts_, int) else config.num_experts_[index]):
                    lora_a = None
                    lora_b = None
            ...
            for idx, lora_layer in enumerate(lora_layer_list):
                if lora_name in lora_layer.loras_:
                    ...
                    if transformer_layer.mlp_.moes_[lora_name].moe == "goe":
                        ...
                        ] = transformer_layer.mlp_.moes_[lora_name].lamb
                        lora_weight_dict[
                            moe_layer_prefix_name + "gate.theta"
                        ] = transformer_layer.mlp_.moes_[lora_name].theta
                        lora_weight_dict[
                            moe_layer_prefix_name + "gate.X"
                        ] = transformer_layer.mlp_.moes_[lora_name].gate_.X
                        lora_weight_dict[
                            moe_layer_prefix_name + "gate.mlp.weight"
                        ] = transformer_layer.mlp_.moes_[lora_name].gate_.mlp.weight
                        lora_weight_dict[
                            moe_layer_prefix_name + "gate.mlp_struct.weight"
                        ] = transformer_layer.mlp_.moes_[lora_name].gate_.mlp_struct.weight
                        lora_weight_dict[
                            moe_layer_prefix_name + "gate.proj.weight"
                        ] = transformer_layer.mlp_.moes_[lora_name].gate_.proj.weight
                        """lora_weight_dict[
                            moe_layer_prefix_name + "gate.edge_index"
                        ] = transformer_layer.mlp_.moes_[lora_name].gate_.edge_index"""
                        for i in range(transformer_layer.mlp_.moes_[lora_name].gate_.n_layers):
                            lora_weight_dict[
                                moe_layer_prefix_name + f"gate.convs.{i}.lin.weight"
                                ] = transformer_layer.mlp_.moes_[lora_name].gate_.convs[i].lin.weight
                    else :
                        lora_weight_dict[
